      ...
      <div style={s.body}>
        ...
        <main style={s.main}>
            ...
            <div style={s.overviewMapThumb}>
              <div style={s.mapThumbInner}>
                <svg width="100%" height="100%" viewBox="0 0 260 160" xmlns="http://www.w3.org/2000/svg">
                  <rect width="260" height="160" fill="#EFF6FF" rx="8"/>
                  {/* simplified India outline suggestion */}
                  <path d="M80 20 Q120 15 160 25 L175 60 Q180 80 170 110 Q160 135 130 145 Q100 150 80 130 Q55 110 50 80 Q45 50 80 20Z" fill="#BFDBFE" stroke="#93C5FD" strokeWidth="1.5"/>
                  <circle cx="130" cy="75" r="5" fill="#2563EB" opacity="0.9"/>
                  <circle cx="110" cy="55" r="3.5" fill="#EF4444" opacity="0.85"/>
                  <circle cx="155" cy="95" r="3.5" fill="#F59E0B" opacity="0.85"/>
                  <circle cx="100" cy="100" r="3" fill="#2563EB" opacity="0.7"/>
                  <circle cx="145" cy="50" r="3" fill="#EF4444" opacity="0.7"/>
                  <circle cx="130" cy="75" r="12" fill="none" stroke="#2563EB" strokeWidth="1" opacity="0.4"/>
                  <circle cx="130" cy="75" r="20" fill="none" stroke="#2563EB" strokeWidth="0.5" opacity="0.2"/>
                </svg>
              </div>
          ...
          <div style={s.statsRow}>
            ...
            } value={stats.totalRefugees.toLocaleString('en-IN')} label="Registered Civilians" color="#EFF6FF" />
            <StatCard icon={
          ...
          <section style={s.sectionBlock}>
            ...
            <div style={s.actionGrid}>
              <ActionCard href="/report-missing" icon="🔍" label="Report Missing Person" desc="File a missing person report with photo and details" urgent />
              <ActionCard href="/flood-prediction" icon="🗺️" label="Disaster Map" desc="Live alerts, camp locations & flood zones" />
              <ActionCard href="/track-report" icon="📋" label="Track Report" desc="Check status of your filed report" />
              <ActionCard href="/register" icon="📱" label="My QR Identity Card" desc="Register & download your emergency QR card" />
            </div>
          </section>
          ...
          <section style={s.sectionBlock}>
            ...
            <div style={s.featureGrid}>
              <FeatureCard icon="🔔" title="Voice Call Alerts" desc="Automated multilingual calls in 12+ Indian languages when disaster strikes near you." />
              <FeatureCard icon="📸" title="AI Face Recognition" desc="Match displaced persons across camps using face-scan technology for faster reunification." />
              <FeatureCard icon="🏕️" title="QR Camp Check-in" desc="Instant camp registration — scan your QR identity card at any relief camp entry point." />
              <FeatureCard icon="👨‍👩‍👧‍👦" title="Family Group Linking" desc="Register dependents once — automatic tracking and notifications for entire family units." />
            </div>
          </section>
        </main>
        ...
        <aside style={s.sidebar}>
          ...
          <div style={s.sideCard}>
            ...
            <div style={s.sideDivider} />
            <div style={s.sideStat}><span style={s.sideStatLabel}>States Affected</span><span style={{ ...s.sideStatVal, color: '#2563EB' }}>4</span></div>
            <div style={s.sideDivider} />
      ...
      <footer style={s.footer}>
        <div style={s.footerInner}>
          <div style={s.footerBrand}>
            <svg width="20" height="20" viewBox="0 0 28 28" fill="none"><path d="M14 2L3 8v7c0 5.55 4.7 10.74 11 12 6.3-1.26 11-6.45 11-12V8L14 2z" fill="#2563EB"/><path d="M10 14l3 3 5-5" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            <span style={s.footerBrandName}>Sahaay Emergency Response</span>
          </div>
...
function StatCard({ icon, value, label, color, accent = '#2563EB', urgent }) {
  return (
...
function ActionCard({ href, icon, label, desc, urgent }) {
  return (
    <Link href={href} style={{ ...sc.actionCard, ...(urgent ? sc.actionCardUrgent : {}) }}>
      <div style={sc.actionIcon}>{icon}</div>
      <div style={sc.actionLabel}>{label}</div>
      <div style={sc.actionDesc}>{desc}</div>
      <div style={{ ...sc.actionArrow, color: urgent ? '#DC2626' : '#2563EB' }}>→</div>
    </Link>
  );
}
...
function FeatureCard({ icon, title, desc }) {
  return (
    <div style={sc.featureCard}>
      <div style={sc.featureIcon}>{icon}</div>
      <div style={sc.featureTitle}>{title}</div>
      <div style={sc.featureDesc}>{desc}</div>
    </div>
  );
}
...
const s = {
  ...
  /* Nav */
  nav: { background: 'white', borderBottom: '1px solid #E2E8F0', padding: '0 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: 56, position: 'sticky', top: 0, zIndex: 200, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' },
  navLeft: { display: 'flex', alignItems: 'center', gap: 36 },
  navLogo: { display: 'flex', alignItems: 'center', gap: 9 },
  navLogoText: { fontSize: 17, fontWeight: 700, color: '#0F172A', letterSpacing: '-0.3px' },
  navBadge: { fontSize: 10, fontWeight: 700, background: '#DCFCE7', color: '#16A34A', padding: '2px 7px', borderRadius: 20, letterSpacing: '0.5px' },
  navLinks: { display: 'flex', gap: 4 },
  navLink: { fontSize: 13.5, color: '#6B7280', padding: '6px 12px', borderRadius: 6, textDecoration: 'none', fontWeight: 500, transition: 'all 0.15s' },
  ...
  navLoginBtn: { fontSize: 13.5, fontWeight: 600, color: 'white', background: '#2563EB', padding: '7px 16px', borderRadius: 7, textDecoration: 'none' },
  ...
  overviewText: { flex: 1 },
  overviewEyebrow: { fontSize: 12, fontWeight: 600, color: '#2563EB', textTransform: 'uppercase', letterSpacing: '0.8px', margin: '0 0 8px' },
  ...
  btnPrimary: { display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 22px', background: '#2563EB', color: 'white', borderRadius: 8, fontSize: 14, fontWeight: 600, textDecoration: 'none', boxShadow: '0 2px 8px rgba(37,99,235,0.3)' },
  ...
  mapThumbInner: { borderRadius: 8, overflow: 'hidden', border: '1px solid #DBEAFE', height: 160 },
  ...
  /* Sidebar cards */
  sideRegisterCard: { background: 'linear-gradient(135deg, #1D4ED8 0%, #2563EB 100%)', borderRadius: 12, padding: '22px 20px', color: 'white' },
  ...
  sideRegisterBtn: { display: 'inline-block', background: 'white', color: '#1D4ED8', fontSize: 13.5, fontWeight: 700, padding: '9px 18px', borderRadius: 7, textDecoration: 'none' },
...
const sc = {
  /* Stat card */
  statCard: { background: '#EFF6FF', border: '1px solid #E5E7EB', borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 },
  ...
  alertDetailsBtn: { flexShrink: 0, fontSize: 13, fontWeight: 600, color: '#2563EB', background: '#EFF6FF', border: '1px solid #BFDBFE', padding: '7px 14px', borderRadius: 7, textDecoration: 'none', alignSelf: 'center' },
